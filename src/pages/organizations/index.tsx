...
function OrganizationListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface[]>(
    ...
    () =>
      getOrganizations({
        relations: ['user', 'asset.count', 'financial_data.count', 'wealth_management_strategy.count'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('wealth_management_strategy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>wealth_management_strategy</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('financial_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.financial_data}</Td>
                    )}
                    {hasAccess('wealth_management_strategy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.wealth_management_strategy}</Td>
                    )}
                    <Td>
